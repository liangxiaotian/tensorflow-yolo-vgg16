    ...
    def inference(self, image):
        ...
        with tf.variable_scope('vgg_16') as scope:
            with tf.variable_scope('conv1') as scope:
                ...
                temp_conv = self.conv2d("conv1_1", image, kernel_size = [3, 3, 3, 64], stride=1)
        ...
        temp_conv = tf.transpose(temp_pool, (0, 3, 1, 2))

        # #全连接层
        local1 = self.local('local1',temp_conv, self.cell_size * self.cell_size * 512, 256, pretrain = False)

        #local1 = tf.nn.dropout(local1, keep_prob=0.5)
        local2 = self.local('local2', local1, 256, 4096, pretrain = False)

        local3 = self.local('local3', local2, 4096,
                            self.cell_size * self.cell_size * (self.num_classes + 5 * self.boxes_per_cell), leaky = False, pretrain = False, train=True)
        #shape = int(local2.get_shape()[0])
        #local3 = tf.reshape(local3, [tf.shape(local3)[0], self.cell_size, self.cell_size, self.num_classes + 5 * self.boxes_per_cell])
        ...
        class_prob = tf.reshape(local3[:, 0:n1], (-1, self.cell_size, self.cell_size, self.num_classes))
        ...
        local3 = tf.concat([class_prob, scales, boxes], 3)
